...
router.post('/requests', requireIdempotencyKey, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const pr = await req.withTenantDb!(async (tx) => {
      ...
      const p = await tx.purchaseRequest.create({
        data: {
          tenantId, prNumber, requestedBy: actorId,
          ...body,
      ...
      await writeAuditLogTx(tx, {
        ...
        resourceType: 'purchase_request', resourceId: p.id,
...
router.patch('/requests/:id/submit', async (req: Request, res: Response, next: NextFunction) => {
  try {
    assertCanPerform(req.tenantContext!, 'update', { type: 'purchase_request', module: 'procurement' });
    const { tenantId, actorId } = req.tenantContext!;

    const updated = await req.withTenantDb!(async (tx) => {
      const pr = await tx.purchaseRequest.findFirst({ where: { id: req.params.id, tenantId } });
      if (!pr) return null;
      if (pr.status !== 'Draft') throw new AppError(422, 'Only Draft requests can be submitted');
      const u = await tx.purchaseRequest.update({ where: { id: pr.id }, data: { status: 'Pending' } });
      await writeAuditLogTx(tx, {
        tenantId, actorId, action: 'submit',
        resourceType: 'purchase_request', resourceId: pr.id,
        before: pr as unknown as Record<string, unknown>,
...
router.post('/orders', requireIdempotencyKey, async (req: Request, res: Response, next: NextFunction) => {
  try {
    assertCanPerform(req.tenantContext!, 'create', { type: 'purchase_request', module: 'procurement' });
    const { tenantId, actorId } = req.tenantContext!;
    ...
    const lines = body.lines.map((l, i) => ({
      ...l, lineNumber: i + 1, amount: l.quantity * l.unitPrice,
    }));
